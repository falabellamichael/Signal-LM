(function () {
  ...
  async function runHybridPlainChat(originalFetch, body) {
    ...
    if ((settings.runtimeMode || 'server') !== 'hybrid' || strategy === 'off') return await callPc(originalFetch, body, settings);
    ...
    try { return await withTimeout(callPc(originalFetch, body, settings), timeoutMs); }
    catch (error) { return await callNative(body, settings); }
  }

  function responseForText(text) {
    return new Response(JSON.stringify({ output: [{ type: 'message', content: String(text || '(No content returned.)') }] }), { status: 200, headers: { 'content-type': 'application/json' } });
  }
  ...
  function install() { installFetchRouter(); }
